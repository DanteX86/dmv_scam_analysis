...
@pass_debug
def analyze_message(ctx, message):
...
@pass_debug
def analyze_file(ctx, file_path):
    ...
    for idx, row in df.iterrows():
        ...
        if features:
            ...
            for name, value in list(features['features'].iloc[0].items())[:5]:
                feature_table.add_row(name, f"{value:.4f}" if isinstance(value, float) else str(value))
...
def inspect_model(ctx):
    ...
    if hasattr(ctx.classifier, 'models') and 'threat_classifier' in ctx.classifier.models:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            # Get feature names
            feature_names = ctx.current_features['feature_names'] if ctx.current_features else [f"feature_{i}" for i in range(len(model.feature_importances_))]
...
@click.argument('message')
@pass_debug
def debug_features(ctx, message):
...
@click.argument('message')
@pass_debug
def explain_prediction(ctx, message):
