...
class AdvancedNLPAnalyzer:
    ...
    def __init__(self, output_dir="./analysis_output"):
        ...
        self.threat_vocabularies = {
            'urgency': [
                ...
                'expire', 'suspend', 'cancel', 'terminate', 'final', 'last'
            ],
            'authority': [
                'dmv', 'department', 'government', 'official', 'agency', 'bureau',
                'administration', 'authority', 'office', 'court', 'legal'
            ],
            'financial': [
                'payment', 'fee', 'fine', 'penalty', 'cost', 'charge', 'bill',
                'amount', 'money', 'pay', 'owe', 'debt', 'credit', 'account'
            ],
            'fear_appeals': [
                'penalty', 'violation', 'illegal', 'arrest', 'jail', 'prison',
                'criminal', 'felony', 'misdemeanor', 'prosecute', 'sue', 'lawsuit'
            ],
            'action_words': [
                'click', 'visit', 'call', 'contact', 'respond', 'reply', 'confirm',
                'verify', 'update', 'provide', 'submit', 'send', 'download'
            ]
        }
    ...
    def extract_features(self, messages_df):
        ...
        if hasattr(messages_df, 'shape') and len(messages_df.shape) == 1:
            # It's a numpy array of text messages
            text_messages = messages_df
        elif hasattr(messages_df, 'empty'):
            # It's a DataFrame
        ...
        for text in text_messages:
            if pd.isna(text):
                ...
                for vocab in sorted(self.threat_vocabularies.keys()):
                    count = sum(1 for word in filtered_words if word in self.threat_vocabularies[vocab])
                ...
                feature_vector.extend([
                    len(text_str),  # message_length
                    len(words),     # word_count
                    len(sent_tokenize(text_str)),  # sentence_count
                    text_str.count('!'),  # exclamation_count
                    text_str.count('?'),  # question_count
                    sum(1 for c in text_str if c.isupper()) / len(text_str) if text_str else 0  # caps_ratio
                ])
    ...
    def get_feature_importance(self, features, labels):
        """
    ...
    def analyze(self, messages):
        ...
        if analysis_results is None or 'error' in analysis_results:
            error_msg = analysis_results.get('error', 'Unable to analyze messages') if analysis_results else 'Unable to analyze messages'
    ...
    def _analyze_sentiment(self, messages_df):
        ...
        if sentiments:
            ...
            aggregate_stats = {
                ...
                'sentiment_distribution': {
                    'positive': len([s for s in sentiments if s['sentiment_category'] == 'positive']),
                    'negative': len([s for s in sentiments if s['sentiment_category'] == 'negative']),
                    'neutral': len([s for s in sentiments if s['sentiment_category'] == 'neutral'])
                },
                'sent_vs_received': {
                    'sent_polarity_mean': np.mean([s['polarity'] for s in sent_messages]) if sent_messages else 0,
                    'received_polarity_mean': np.mean([s['polarity'] for s in received_messages]) if received_messages else 0,
                    'sentiment_asymmetry': self._calculate_sentiment_asymmetry(sent_messages, received_messages)
                }
            }
        else:
            aggregate_stats = {}
    ...
    def _calculate_sentiment_asymmetry(self, sent_messages, received_messages):
        ...
        return abs(sent_polarity - received_polarity)
    
    def _create_sentiment_timeline(self, sentiments):
    ...
    def _extract_entities(self, messages_df):
        ...
        for entity_type, entities in all_entities.items():
            if entities:
                entity_stats[entity_type] = {
                    'count': len(entities),
                    'unique_count': len(set(e['value'] if 'value' in e else e['entity'] for e in entities)),
    ...
    def _get_entity_context(self, text, entity, window=20):
    ...
    def _get_most_common_entities(self, entities, entity_type):
    ...
    def _identify_suspicious_entities(self, all_entities):
    ...
    def _analyze_vocabulary(self, messages_df):
    ...
    def _calculate_threat_vocabulary_score(self, threat_usage):
    ...
    def _analyze_linguistic_patterns(self, messages_df):
        ...
        for _, message in messages_df.iterrows():
            ...
            patterns['capitalization_patterns']['all_caps_words'] = patterns['capitalization_patterns'].get('all_caps_words', 0)
            patterns['capitalization_patterns']['all_caps_words'] += len([word for word in text.split() if word.isupper() and len(word) > 1])
        ...
        if patterns['message_lengths']:
            ...
            patterns['aggregate_stats'] = {
                ...
                'error_rate': len(patterns['grammatical_errors']) / len(patterns['message_lengths'])
            }
        
        return patterns
    
    def _detect_deception_indicators(self, messages_df):
    ...
    def _calculate_deception_score(self, indicators):
    ...
    def _analyze_urgency_patterns(self, messages_df):
    ...
    def _extract_semantic_topics(self, messages_df):
        ...
        topic_keywords = {
            'government_services': ['dmv', 'license', 'registration', 'renewal', 'government', 'department'],
            'financial_transactions': ['payment', 'fee', 'cost', 'money', 'credit', 'debit', 'bank'],
            'legal_threats': ['violation', 'penalty', 'court', 'legal', 'arrest', 'prosecution'],
            'identity_verification': ['verify', 'confirm', 'identity', 'ssn', 'personal', 'information'],
            'technical_support': ['system', 'update', 'technical', 'error', 'maintenance', 'server']
        }
        ...
        return {
            ...
            'dominant_topics': sorted(topic_stats.items(), key=lambda x: x[1]['prevalence'], reverse=True)[:3]
        }
    
    def generate_nlp_report(self, contact_identifier, nlp_analysis):
        """
    ...
    def _assess_nlp_risk(self, nlp_analysis):
        ...
        if suspicious_entities.get('suspicious_domains'):
            risk_factors.append(f"Suspicious domains detected: {len(suspicious_entities['suspicious_domains'])}")
            risk_score += 25
        
        if suspicious_entities.get('suspicious_phone_numbers'):
            risk_factors.append(f"International phone numbers detected: {len(suspicious_entities['suspicious_phone_numbers'])}")
            risk_score += 20
    ...
    def _generate_nlp_recommendations(self, nlp_analysis):
    ...
    def _generate_nlp_summary(self, report, contact_identifier):
        ...
        with open(summary_file, 'w') as f:
            ...
            if sentiment:
                ...
                f.write(f"Overall Sentiment: {aggregate.get('overall_polarity_mean', 0):.2f} (polarity)\n")
            
            # Entity summary
            entities = nlp_analysis.get('entity_extraction', {})
            if entities:
                entity_stats = entities.get('entity_statistics', {})
                f.write(f"Extracted Entities: {sum(stats.get('count', 0) for stats in entity_stats.values())}\n")
