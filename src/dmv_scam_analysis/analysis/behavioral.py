...
class BehavioralAnalyzer:
    ...
    def __init__(self, output_dir="./analysis_output"):
        ...
        self.version = "1.0.0"
        self.patterns = []
        self.available_indicators = ['urgency_language', 'authority_impersonation', 'high_behavioral_score', 'volume_anomaly', 'timing_anomaly']
    ...
    def analyze(self, messages_df):
        ...
        # Return results in format expected by CLI
        return {
    ...
    def extract_iocs(self, messages, threshold=0.7):
        """
    ...
    def _extract_indicators(self, patterns):
        """
    ...
    def detect_patterns(self, messages_df):
        """
    ...
    def _analyze_communication_patterns(self, messages_df):
        ...
        patterns = {
            ...
            'avg_message_length': messages_df['text'].str.len().mean() if 'text' in messages_df.columns else 0,
            'communication_style': self._analyze_communication_style(messages_df)
        }
        
        return patterns
    
    def _analyze_communication_style(self, messages_df):
        ...
        style_metrics = {
            'caps_usage': sum(1 for c in all_text if c.isupper()) / len(all_text) if all_text else 0,
            'punctuation_density': sum(1 for c in all_text if c in '!?.') / len(all_text) if all_text else 0,
            'urgency_keywords': sum(1 for word in ['urgent', 'immediate', 'now', 'asap'] if word in all_text.lower()),
            'authority_keywords': sum(1 for word in ['dmv', 'government', 'official'] if word in all_text.lower())
        }
        
        return style_metrics
    
    def _detect_anomaly_patterns(self, messages_df):
    ...
    def _calculate_behavioral_score(self, messages_df):
        ...
        if 'text' in messages_df.columns:
            ...
            if not text_messages.empty:
                ...
                # Threat keywords
                threat_keywords = ['urgent', 'immediate', 'suspended', 'dmv', 'government', 'penalty']
    ...
    def _analyze_source_patterns(self, messages_df):
        ...
        source_stats = {
            ...
            'source_ratios': {source: count/total_messages for source, count in source_counts.items()}
        }
    ...
    def _analyze_threat_clusters(self, messages_df):
        """Analyze threat clustering patterns in messages"""
        if 'text' not in messages_df.columns:
            return {'error': 'no_text_column'}
        
        # Simple clustering based on content similarity and threat indicators
        threat_keywords = ['urgent', 'immediate', 'dmv', 'government', 'penalty', 'suspended', 'click', 'verify']
        ...
        for idx, row in messages_df.iterrows():
            ...
            if found_keywords:
                ...
                clusters[cluster_key]['messages'].append({
                    'index': idx,
        ...
        for cluster_key, cluster in clusters.items():
            ...
            for msg in cluster['messages']:
                source = msg['source']
                cluster['source_distribution'][source] = cluster['source_distribution'].get(source, 0) + 1
        
        return clusters
        
    def analyze_temporal_patterns(self, messages_df):
        """
        ...
        if datetime_col is None:
            ...
            messages_df['datetime'] = pd.date_range(start='2025-01-01', periods=len(messages_df), freq='1H')
        else:
    ...
    def _analyze_hourly_patterns(self, messages_df):
    ...
    def _analyze_weekly_patterns(self, messages_df):
        ...
        total_messages = len(messages_df)
        weekend_ratio = weekly_stats['weekend_messages'] / total_messages if total_messages > 0 else 0
    ...
    def _detect_message_bursts(self, messages_df):
        ...
        for timestamp, count in bursts.items():
            if not pd.isna(count):
                burst_events.append({
                    ...
                    'threshold': float(burst_threshold[timestamp]) if not pd.isna(burst_threshold[timestamp]) else 0,
                    'severity': self._calculate_burst_severity(count, burst_threshold[timestamp])
                })
        
        return {
            'burst_events': burst_events,
            'total_bursts': len(burst_events),
            'max_burst_intensity': max([e['message_count'] for e in burst_events]) if burst_events else 0
        }
    
    def _calculate_burst_severity(self, count, threshold):
    ...
    def _detect_timing_anomalies(self, messages_df):
    ...
    def _analyze_response_patterns(self, messages_df):
        ...
        return {
            ...
            'rapid_responses': [rt for rt in response_times if rt['response_time_seconds'] < 60],  # < 1 minute
            'delayed_responses': [rt for rt in response_times if rt['response_time_seconds'] > 3600]  # > 1 hour
        }
    
    def detect_automation_indicators(self, messages_df):
        """
        ...
        automation_score = self._calculate_automation_score(automation_indicators)
        automation_indicators['overall_automation_score'] = automation_score
        
        return automation_indicators
    
    def _analyze_timing_regularity(self, messages_df):
    ...
    def _interpret_timing_regularity(self, regularity_score, interval_diversity):
    ...
    def _analyze_content_patterns(self, messages_df):
        ...
        duplicate_ratio = 1 - (unique_messages / len(message_texts))
        ...
        return {
            ...
            'analysis': self._interpret_content_similarity(average_similarity, duplicate_ratio)
        }
    ...
    def _interpret_content_similarity(self, similarity_score, duplicate_ratio):
    ...
    def _analyze_volume_patterns(self, messages_df):
        ...
        return {
            ...
            'analysis': 'HIGH_VOLUME_CONSISTENCY' if consistency_score > 0.7 else 'VARIABLE_VOLUME_PATTERN'
        }
    
    def _analyze_response_predictability(self, messages_df):
        ...
        response_ratio = len(received_messages) / len(sent_messages) if len(sent_messages) > 0 else 0
        ...
        return {
            ...
            'analysis': 'HIGH_PREDICTABILITY' if predictability_score > 0.8 else 'VARIABLE_RESPONSE_PATTERN'
        }
    
    def _calculate_automation_score(self, indicators):
    ...
    def generate_behavioral_report(self, contact_identifier, temporal_analysis, automation_analysis):
        """
        ...
        report = {
            ...
            'recommendations': self._generate_behavioral_recommendations(temporal_analysis, automation_analysis)
        }
    ...
    def _assess_behavioral_risk(self, temporal_analysis, automation_analysis):
    ...
    def _generate_behavioral_recommendations(self, temporal_analysis, automation_analysis):
        ...
        if temporal_analysis and temporal_analysis.get('burst_detection', {}).get('total_bursts', 0) > 2:
            recommendations.append({
                'priority': 'MEDIUM',
        ...
        if temporal_analysis and temporal_analysis.get('response_patterns', {}).get('rapid_responses'):
    ...
    def _generate_behavioral_summary(self, report, contact_identifier):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Behavioral Risk Score: {risk_assessment.get('behavioral_risk_score', 0)}/100\n")
            ...
            f.write(f"Automation Likelihood: {automation_score:.2f} ({automation_score * 100:.1f}%)\n\n")
...
def main():
    ...
    try:
        ...
        print(f"✓ Automation Score: {report['automation_indicators']['overall_automation_score']:.2f}")
