...
class ThreatDashboard:
    ...
    def generate_dashboard_html(
        ...
        if self.threats_data:
            ...
            fig.add_trace(
                go.Bar(x=dates, y=high_counts, name="High Risk", marker_color="red"),
                row=3,
                col=1,
            )
            fig.add_trace(
                go.Bar(
                    x=dates, y=medium_counts, name="Medium Risk", marker_color="orange"
                ),
                row=3,
                col=1,
            )
            fig.add_trace(
                go.Bar(x=dates, y=low_counts, name="Low Risk", marker_color="green"),
                row=3,
                col=1,
            )
        ...
        fig.add_trace(
            go.Table(
                header=dict(
                    values=["Metric", "Current Value", "Status"],
        ...
        fig.update_layout(
            ...
            showlegend=True,
            title_text=(
                f"DMV Scam Analysis - Real-time Threat Dashboard "
        ...
        # 'template' is currently unused; reserved for future extension
    ...
    def _generate_basic_html_dashboard(self, output_file: str) -> str:
        ...
        high_risk_count = len(
            [t for t in recent_threats if t["classification"] == "high_risk"]
        )
        medium_risk_count = len(
            [t for t in recent_threats if t["classification"] == "medium_risk"]
        )
        low_risk_count = len(
    ...
    def _calculate_performance_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        if "response_time" in self.metrics_data and self.metrics_data["response_time"]:
            recent_response_times = [
                m["value"] for m in self.metrics_data["response_time"][-10:]
            ]
        ...
        return {
            ...
            "summary": {
                ...
                "high_risk_count": len(
                    [
                        t
                        for t in self.threats_data[-100:]
                        if t["classification"] == "high_risk"
                    ]
                ),
            },
