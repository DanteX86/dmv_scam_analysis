...
class LogMaintenance:
    ...
    def get_log_stats(self) -> dict:
        ...
        for log_file in self.log_dir.rglob("*.log*"):
            ...
            stats["by_type"][log_type]["size"] += file_size
...
def main():
    ...
    parser.add_argument("--keep-days", type=int, default=30,
                      help="Keep archives for this many days")
    parser.add_argument("--stats", action="store_true",
                      help="Show log statistics")
    parser.add_argument("--rotate", action="store_true",
                      help="Rotate active log files")
    ...
    try:
        ...
        if args.stats:
            ...
            if stats['oldest_file']:
                print(f"Oldest file: {stats['oldest_file'][0]} "
                      f"({stats['oldest_file'][1].isoformat()})")
            if stats['newest_file']:
                print(f"Newest file: {stats['newest_file'][0]} "
