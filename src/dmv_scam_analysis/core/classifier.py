...
class MLThreatClassifier:
    ...
    def __init__(self, output_dir="./analysis_output"):
        ...
        self.patterns = []
        self.available_indicators = ['urgency_language', 'authority_impersonation', 'high_behavioral_score']
    ...
    def extract_ml_features(self, messages_df, include_labels=False):
        """
    ...
    def _extract_text_features(self, messages_df):
        ...
        text_features['avg_word_length'] = np.mean([len(word) for word in all_text.split()]) if all_text.split() else 0
        text_features['unique_words_ratio'] = len(set(all_text.split())) / len(all_text.split()) if all_text.split() else 0
        ...
        for category, keywords in threat_keywords.items():
            ...
            text_features[f'{category}_ratio'] = count / len(all_text.split()) if all_text.split() else 0
        ...
        text_features['has_suspicious_domain'] = 1 if any(tld in all_text.lower() for tld in ['.vip', '.tk', '.ml', '.ga']) else 0
        ...
        text_features['caps_ratio'] = sum(1 for c in all_text if c.isupper()) / len(all_text) if all_text else 0
        
        return text_features
    
    def _extract_temporal_features(self, messages_df):
        ...
        temporal_features['messages_per_day'] = len(messages_df) / max(1, temporal_features['unique_days'])
        ...
        temporal_features['night_messages_ratio'] = hourly_dist[night_hours].sum() / len(messages_df) if len(messages_df) > 0 and night_hours else 0
        temporal_features['business_hours_ratio'] = hourly_dist[business_hours].sum() / len(messages_df) if len(messages_df) > 0 and business_hours else 0
        temporal_features['weekend_messages_ratio'] = messages_df['is_weekend'].sum() / len(messages_df) if len(messages_df) > 0 else 0
        
        # Time interval features
        if len(messages_df) > 1:
            time_diffs = messages_df.sort_values('datetime')['datetime'].diff().dt.total_seconds().dropna()
            ...
            temporal_features['interval_regularity'] = 1 / (1 + time_diffs.std() / time_diffs.mean()) if time_diffs.mean() > 0 else 0
        else:
        ...
        temporal_features['burst_hours_count'] = (hourly_messages > hourly_messages.mean() + 2 * hourly_messages.std()).sum()
        
        return temporal_features
    
    def _extract_behavioral_features(self, messages_df):
        ...
        if 'is_from_me' in messages_df.columns:
            ...
            behavioral_features['received_ratio'] = received_count / total_count if total_count > 0 else 0
        ...
        if 'text' in messages_df.columns:
            ...
            if not text_messages.empty:
                ...
                behavioral_features['length_consistency'] = 1 / (1 + lengths.std() / lengths.mean()) if lengths.mean() > 0 else 0
                ...
                behavioral_features.update({
                    'avg_message_length': 0, 'message_length_std': 0, 'max_message_length': 0,
                    'length_consistency': 0, 'very_short_messages': 0, 'very_long_messages': 0
                })
        ...
        if 'is_from_me' in messages_df.columns and 'datetime' in messages_df.columns:
            ...
            if len(sent_msgs) > 0 and len(received_msgs) > 0:
                ...
                for _, sent_msg in sent_msgs.iterrows():
                    subsequent_received = received_msgs[received_msgs['datetime'] > sent_msg['datetime']]
                    if not subsequent_received.empty:
                        response_time = (subsequent_received.iloc[0]['datetime'] - sent_msg['datetime']).total_seconds()
                ...
                if response_times:
                    ...
                    behavioral_features['rapid_responses'] = sum(1 for rt in response_times if rt < 60) / len(response_times)
                else:
                    behavioral_features.update({'avg_response_time': 0, 'response_time_std': 0, 'rapid_responses': 0})
            else:
                behavioral_features.update({'avg_response_time': 0, 'response_time_std': 0, 'rapid_responses': 0})
        
        return behavioral_features
    
    def _extract_statistical_features(self, messages_df):
        """Extract statistical features from message data"""
        statistical_features = {}
        
        # Basic counts
        statistical_features['total_messages'] = len(messages_df)
        statistical_features['non_empty_messages'] = messages_df['text'].notna().sum() if 'text' in messages_df.columns else 0
        ...
        if 'datetime' in messages_df.columns:
            time_span = (messages_df['datetime'].max() - messages_df['datetime'].min()).total_seconds()
            statistical_features['communication_span_hours'] = time_span / 3600
            statistical_features['message_density'] = len(messages_df) / max(1, time_span / 3600)  # messages per hour
        else:
            statistical_features.update({'communication_span_hours': 0, 'message_density': 0})
        
        return statistical_features
    
    def _extract_message_features(self, message):
        ...
        features['has_suspicious_domain'] = 1 if any(tld in text.lower() for tld in ['.vip', '.tk', '.ml', '.ga']) else 0
        ...
        if 'readable_date' in message:
            try:
                ...
                features['is_business_hours'] = 1 if msg_datetime.hour in [9, 10, 11, 12, 13, 14, 15, 16, 17] else 0
            except:
                features.update({'hour': 12, 'day_of_week': 0, 'is_weekend': 0, 'is_night': 0, 'is_business_hours': 1})
        else:
            features.update({'hour': 12, 'day_of_week': 0, 'is_weekend': 0, 'is_night': 0, 'is_business_hours': 1})
    ...
    def _generate_synthetic_labels(self, messages_df):
    ...
    def train_threat_classifier(self, training_data):
        """
        ...
        if successful_models:
            best_model_name = max(
                successful_models,
                key=lambda k: training_results[k]['f1_score']
            )
    ...
    def predict_threat_classification(self, messages_df):
        """
        ...
        try:
            ...
            if probabilities is not None and probabilities.shape[1] > 1:
                max_threat_prob = np.max(probabilities[:, 1:])  # Exclude benign category for all messages
    ...
    def _categorize_threat_risk(self, max_probability):
    ...
    def detect_anomalies(self, messages_df):
        """
    ...
    def _calculate_anomaly_likelihood(self, anomaly_results):
    ...
    def _identify_primary_concerns(self, outlier_features):
    ...
    def generate_ml_report(self, contact_identifier, ml_results):
        """
    ...
    def _assess_ml_risk(self, ml_results):
        ...
        if 'predictions' in ml_results:
            ...
            if any(pred != 'benign' for pred in predictions):
                ...
                risk_factors.append(f"Threat classification detected: {', '.join(set(threat_types))}")
                risk_score += 30
    ...
    def _generate_ml_recommendations(self, ml_results):
    ...
    def _generate_ml_summary(self, report, contact_identifier):
        ...
        with open(summary_file, 'w') as f:
            ...
            if 'anomaly_detection' in ml_analysis:
                ...
                anomaly_likelihood = anomaly_data.get('overall_assessment', {}).get('anomaly_likelihood', 0)
    ...
    def train(self, training_data=None, labels=None, texts=None):
    ...
    def train_ensemble(self, training_data=None, model_types=None, texts=None, labels=None, n_models=None):
    ...
    def update_model(self, texts=None, labels=None):
    ...
    def explain_prediction(self, text):
        ...
        for feature, value in features.items():
            if value > 0 and feature_importance.get(feature, 0) > 0.05:  # Only show significant features
    ...
    try:
        ...
        if results:
            ...
            if 'predictions' in results:
                print(f"✓ Classification: {results['predictions'].get('predictions', ['Unknown'])[0]}")
                if 'max_threat_probability' in results['predictions']:
                    print(f"✓ Confidence: {results['predictions']['max_threat_probability']:.1%}")
