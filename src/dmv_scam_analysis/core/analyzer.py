...
class CampaignAnalyzer:
    ...
    def __init__(self, campaign_name, output_dir="./analysis_output"):
        """
    ...
    def analyze_campaign(self, data_source, campaign_type=None):
        """
        ...
        try:
            ...
            # Update metadata
            self.results['campaign_metadata'].update({
            ...
            for contact in messages_df['contact_id'].unique():
                ...
                if contact_analysis:
                    self.results['contacts_analyzed'].append({
                        'contact_id': contact,
                        'risk_level': contact_analysis['risk_assessment']['risk_level'],
                        'automation_score': contact_analysis['automation_indicators']['overall_automation_score']
                    })
                    
                    # Track high-risk contacts
                    if contact_analysis['risk_assessment']['risk_level'] == 'HIGH':
                        self.results['high_risk_contacts'].append({
                            'contact_id': contact,
                            'risk_score': contact_analysis['risk_assessment']['behavioral_risk_score'],
                    ...
                    if contact_analysis['automation_indicators']['overall_automation_score'] > 0.7:
                        self.results['automation_indicators'].append({
                            'contact_id': contact,
                            'automation_score': contact_analysis['automation_indicators']['overall_automation_score'],
    ...
    def _analyze_contact(self, contact_id, messages_df):
    ...
    def _calculate_campaign_stats(self):
        """Calculate campaign-wide statistics"""
        ...
        self.results['campaign_stats'] = {
            ...
            'risk_distribution': {
                'HIGH': len([c for c in self.results['contacts_analyzed'] if c['risk_level'] == 'HIGH']),
                'MEDIUM': len([c for c in self.results['contacts_analyzed'] if c['risk_level'] == 'MEDIUM']),
                'LOW': len([c for c in self.results['contacts_analyzed'] if c['risk_level'] == 'LOW'])
            }
        }
    ...
    def _generate_campaign_report(self):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Campaign Type: {self.results['campaign_metadata'].get('campaign_type', 'Unknown')}\n")
            ...
            # Key statistics
            stats = self.results['campaign_stats']
...
def configure_input_output(data_source, output_dir, input_format=None, output_format='json'):
    """
    ...
    """
    def load_data(file_path, fmt=None):
    ...
    output_file_base = output_dir_path / f"campaign_analysis"
...
Example usage:
  ...
  %(prog)s --input messages.json --name "Email_Campaign_01" --type email
    ...
    parser.add_argument(
        "--input",
        required=True,
        help="Path to campaign data file (JSON or CSV)"
    )
    parser.add_argument(
        "--name",
        required=True,
    ...
    parser.add_argument(
        "--type",
        help="Campaign type (e.g., sms, email)",
        default=None
    )
    parser.add_argument(
        "--input-format",
