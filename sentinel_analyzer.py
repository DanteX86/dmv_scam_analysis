...
class SentinelAnalyzer:
    ...
    def run_ml_analysis(self, data, contact_id="unknown", train_mode=False):
        ...
        try:
            ...
            if 'predictions' in results:
                classification = results['predictions'].get('predictions', ['Unknown'])[0]
                confidence = results['predictions'].get('max_threat_probability', 0)
                print(f"✓ ML Analysis Complete - Classification: {classification} ({confidence:.1%})")
    ...
    def run_full_analysis(self, data, contact_id="unknown", train_ml=False):
        ...
        report_file = self.output_dir / f"sentinel_analysis_{contact_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
Examples:
  ...
  python sentinel_analyzer.py --input data.json --output ./custom_output --full
    ...
    if args.full:
        ...
        if not any([args.nlp, args.ml, args.behavioral, args.ioc, args.preprocess, args.visualize]):
