...
def generate_statistical_outliers():
    ...
    long_text = (
        "URGENT NOTICE FROM DMV: "
        + "This is a very long scam message that contains repetitive content to make it appear legitimate while actually being fraudulent. "
        * 20
    )
    messages.append(
        {
...
def run_anomaly_test(test_name, messages, classifier):
    ...
    if len(messages) > 0:
        ...
        print(
            f"Message lengths - Min: {min(lengths)}, Max: {max(lengths)}, Avg: {np.mean(lengths):.1f}"
        )
        ...
        for i, msg in enumerate(messages[:3]):
            print(
                f"  {i+1}. [{msg['readable_date']}] {msg['text'][:80]}{'...' if len(msg['text']) > 80 else ''}"
            )
    ...
    try:
        ...
        print("\n📊 Detailed Analysis:")
        print(
            f"  Isolation Forest - Anomaly detected: {iso_forest.get('is_anomaly', False)}"
        )
        print(
            f"  Isolation Forest - Anomaly score: {iso_forest.get('anomaly_score', 0):.3f}"
        )
        print(
            f"  Statistical outliers count: {statistical_outliers.get('outlier_count', 0)}"
        )
        ...
        if statistical_outliers.get("outlier_features"):
            ...
            for feature in statistical_outliers["outlier_features"][:5]:  # Show top 5
                print(
                    f"    - {feature['feature']}: z-score={feature['z_score']:.2f}, value={feature['value']:.2f}"
                )
...
def main():
    ...
    test_scenarios = [
        ...
        (
            "Burst Messaging (Many messages in short time)",
            generate_burst_messaging_pattern(),
        ),
        (
            "Unusual Timing Patterns (Messages at odd hours)",
            generate_unusual_timing_pattern(),
        ),
        (
            "Statistical Outliers in Message Length/Content",
            generate_statistical_outliers(),
        ),
    ]
    ...
    print(
        f"{'Scenario':<45} {'Anomaly %':<12} {'Isolation Forest':<18} {'Outliers':<10}"
    )
    print("-" * 85)
    ...
    for test_name, result in all_results.items():
        ...
        print(
            f"{test_name:<45} {anomaly_pct:<12}% {is_anomaly:<18} {outlier_count:<10}"
        )
    ...
    if all_results:
        ...
        anomaly_scores = {
            name: result.get("overall_assessment", {}).get("anomaly_likelihood", 0)
        ...
        print(
            f"📈 Highest anomaly likelihood: {highest_anomaly} ({anomaly_scores[highest_anomaly]}%)"
        )
        print(
            f"📉 Lowest anomaly likelihood: {lowest_anomaly} ({anomaly_scores[lowest_anomaly]}%)"
        )
        ...
        if all_concerns:
            ...
            for concern_type, count in sorted(
                concern_types.items(), key=lambda x: x[1], reverse=True
            ):
