...
def run_classification_tests():
    ...
    for dataset in datasets:
        ...
        try:
            ...
            print(f"  ✓ Extracted {len(features['feature_names'])} features from {features['message_count']} messages")
            ...
            if not training_results or 'error' in training_results:
                print(f"  ❌ Training failed for {dataset} dataset: {training_results.get('error', 'unknown error')}")
                continue
            ...
            if 'error' in predictions:
                print(f"  ❌ Prediction failed for {dataset} dataset: {predictions['error']}")
                continue
            ...
            if best_model in training_results.get('training_results', {}):
                accuracy = training_results['training_results'][best_model].get('accuracy', 0)
    ...
    if results:
        ...
        for dataset, result in results.items():
            ...
            if best_model in training_res.get('training_results', {}):
                accuracy = training_res['training_results'][best_model].get('accuracy', 0)
                print(f"  • {dataset}: {accuracy:.3f} accuracy ({result['message_count']} messages)")
            else:
        ...
        try:
            ...
            if 'mixed' in results:
                ...
                if test_df is not None:
                    ...
                    feature_data = classifier.extract_ml_features(test_df, include_labels=True)
                    ensemble_results = classifier.train_ensemble(feature_data)
                    
                    if 'error' not in ensemble_results:
                        print(f"  ✓ Ensemble training successful with {ensemble_results.get('ensemble_size', 0)} models")
                        
                        # Test ensemble prediction
                        ensemble_predictions = classifier.predict_ensemble(test_texts)
                        if 'error' not in ensemble_predictions:
                            print(f"  ✓ Ensemble prediction successful for {len(test_texts)} samples")
                        else:
                            print(f"  ❌ Ensemble prediction failed: {ensemble_predictions['error']}")
                    else:
        ...
        try:
            if 'scam' in results:
                ...
                if scam_df is not None:
                    anomaly_results = classifier.detect_anomalies(scam_df.head(10))  # Test with first 10 messages
