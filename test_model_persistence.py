...
def test_model_persistence():
    ...
    try:
        ...
        feature_data = classifier.extract_ml_features(
            test_messages, include_labels=True
        )
        if not feature_data:
        ...
        if "error" in initial_predictions:
            raise Exception(
                f"Initial prediction failed: {initial_predictions['error']}"
            )

        print("✅ Initial predictions successful")
        print(f"   Predictions: {initial_predictions.get('predictions', [])}")
        print(
            f"   Max threat probability: {initial_predictions.get('max_threat_probability', 0):.3f}"
        )
        ...
        try:
            ...
            if "error" not in failed_predictions:
                print(
                    "⚠️  Warning: Predictions should have failed after clearing models"
                )
            else:
        ...
        if "error" in loaded_predictions:
            raise Exception(
                f"Predictions failed after loading: {loaded_predictions['error']}"
            )

        print("✅ Predictions work after loading")
        print(f"   Predictions: {loaded_predictions.get('predictions', [])}")
        print(
            f"   Max threat probability: {loaded_predictions.get('max_threat_probability', 0):.3f}"
        )
        ...
        if conf_diff < 0.001:  # Allow small floating point differences
            ...
            print(
                f"⚠️  Confidence scores differ: {initial_conf:.3f} vs {loaded_conf:.3f}"
            )
        ...
        print(
            f"Adding {len(incremental_data)} new messages for incremental learning..."
        )
        ...
        update_result = new_classifier.update_model(
            texts=incremental_texts, labels=incremental_labels
        )
        ...
        incremental_predictions = new_classifier.predict_threat_classification(
            incremental_data
        )
        if "error" not in incremental_predictions:
            print("✅ Predictions work on incremental data")
            print(
                f"   New predictions: {incremental_predictions.get('predictions', [])}"
            )
        else:
            print(
                f"⚠️  Incremental predictions failed: {incremental_predictions['error']}"
            )
        ...
        print(
            f"   Score range: {min(array_predictions):.3f} - {max(array_predictions):.3f}"
        )
