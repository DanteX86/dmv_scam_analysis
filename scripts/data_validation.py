...
class DataValidator:
    ...
    def validate_dataframe(self, df: pd.DataFrame, dataset_name: str) -> Dict:
        ...
        results["checks"].append({
            ...
            "details": null_counts[null_counts > 0].to_dict() if has_nulls else "No null values found"
        })
        ...
        if 'text' in df.columns:
            ...
            if empty_text_count > 0:
                empty_text_issues.append(f"Found {empty_text_count} messages with empty text")
        
        results["checks"].append({
            "check": "empty_text",
            "passed": len(empty_text_issues) == 0,
            "details": empty_text_issues if empty_text_issues else "No empty text found"
        })
        ...
        results["checks"].append({
            ...
            "details": type_mismatches if type_mismatches else "All data types match expected"
        })
        ...
        results["checks"].append({
            ...
            "details": range_violations if range_violations else "All values within expected ranges"
        })
...
def main():
    ...
    try:
        ...
        processed_results = validator.validate_dataframe(processed_data, "processed_messages")
