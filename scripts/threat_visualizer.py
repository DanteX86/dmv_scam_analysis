...
class ThreatVisualizationSuite:
    ...
    def _generate_sample_data(self) -> Dict[str, Any]:
        ...
        message_counts[50:70] = np.random.poisson(
            lam=8, size=20
        )  # Spike during campaign
    ...
    def create_threat_timeline(self) -> None:
        ...
        ax1.plot(
            self.threat_data["timeline_dates"],
        ...
        ax1.set_title(
            "DMV Scam Campaign: Message Activity Timeline",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        ax2.scatter(
            [t.hour + t.minute / 60 for t in ioc_times],
            ioc_y_positions,
            s=100,
            c=["#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#17becf"],
            alpha=0.8,
        )
        ...
        ax2.set_title(
            "Threat Intelligence Discovery Timeline",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.output_dir}/threat_timeline.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def create_risk_assessment_dashboard(self) -> None:
        ...
        fig.add_trace(
            go.Bar(
                x=self.threat_data["risk_categories"],
        ...
        fig.add_trace(
            go.Pie(
                labels=list(self.threat_data["geographic_data"].keys()),
        ...
        # Save as HTML
        pyo.plot(
            fig, filename=f"{self.output_dir}/risk_dashboard.html", auto_open=False
        )
    ...
    def create_threat_intelligence_network(self) -> None:
        ...
        pyo.plot(
            fig, filename=f"{self.output_dir}/threat_network.html", auto_open=False
        )
    ...
    def create_detection_analytics(self) -> None:
        ...
        colors = [
            "#d62728" if score < 0.85 else "#2ca02c" for score in confidence_scores
        ]
        ...
        ax2.axhline(
            y=0.85, color="orange", linestyle="--", alpha=0.7, label="Threshold"
        )
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        ax4.set_title(
            "Risk Score Evolution During Investigation", fontsize=14, fontweight="bold"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.output_dir}/detection_analytics.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def create_executive_summary_visual(self) -> None:
        ...
        fig.update_layout(
            title_text="DMV Scam Campaign: Executive Dashboard", title_x=0.5, height=600
        )

        pyo.plot(
            fig, filename=f"{self.output_dir}/executive_dashboard.html", auto_open=False
        )

        print(
            f"✓ Executive dashboard saved: {self.output_dir}/executive_dashboard.html"
        )
    ...
    def create_visualization_index(self) -> None:
        ...
        print(
            f"✓ Visualization index saved: {self.output_dir}/visualization_index.html"
        )
...
ThreatVisualizer = ThreatVisualizationSuite
