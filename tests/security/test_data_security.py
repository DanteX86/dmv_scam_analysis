    ...
    for dir_path in data_dirs:
        if os.path.exists(dir_path):
            ...
            assert mode in [
                ...
                0o750,
                0o700,
            ], f"Insecure directory permissions: {mode:o}"
            ...
            for root, _, files in os.walk(dir_path):
                for file in files:
                    ...
                    assert mode in [
                        ...
                        0o640,
                        0o600,
                    ], f"Insecure file permissions: {mode:o}"
...
def test_config_security():
    ...
    config_files = ["./config/settings.yaml", "./config/analysis_config_template.yaml"]
        ...
        if os.path.exists(config_file):
            ...
            def check_dict(d, path=""):
                for k, v in d.items():
                    ...
                    assert not any(
                        s in k.lower()
                        for s in ["password", "secret", "key", "token", "credential"]
...
def test_backup_security():
    ...
    backup_file = os.path.join(
        backup_dir, f"test_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def test_log_security():
    ...
    log_dirs = ["./logs/app", "./logs/audit", "./logs/error", "./logs/performance"]
                                ...
                                for match in matches:
                                    context = content[
                                        max(0, match.start() - 20) : min(
                                            len(content), match.end() + 20
                                        )
                                    ]
                                    assert (
                                        False
                                    ), f"Sensitive data found in log: {context}"
