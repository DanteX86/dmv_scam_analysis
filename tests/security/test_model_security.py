...
from tests.utils.test_helpers import create_test_dataset, create_test_embeddings
...
def test_model_output_validation():
    ...
    for _, message in test_messages.iterrows():
        ...
        assert all(
            key in analysis
            for key in ["threat_score", "confidence", "indicators", "analysis_id"]
        )
...
def test_model_versioning_security():
    ...
    def get_model_hash(model_data):
        ...
        return hashlib.sha256(
            json.dumps(model_data, sort_keys=True).encode()
        ).hexdigest()
