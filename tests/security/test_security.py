...
def test_pii_detection():
    ...
    test_cases = [
        {"input": "My SSN is 123-45-6789", "should_detect": True, "pii_type": "ssn"},
        {
        ...
        {"input": "Regular text without PII", "should_detect": False, "pii_type": None},
    ]
...
def test_input_sanitization():
    ...
    for input_str in malicious_inputs:
        ...
        assert not any(
            keyword.lower() in sanitized.lower()
...
async def test_rate_limiting():
    ...
    limiter = RateLimiter(max_requests=5, time_window=60)  # 1 minute
...
@pytest.mark.parametrize(
    "token", ["Bearer valid_token", "Bearer invalid_token", "Invalid format", None]
)
...
def test_dependency_security():
    ...
    ), f"Found security vulnerabilities: {vulnerabilities}"
