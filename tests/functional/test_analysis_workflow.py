...
def analysis_components(test_data_dir):
    ...
    return {
        "extractor": MessageExtractor(db_path=str(dummy_db), output_dir=str(test_data_dir)),
...
def test_incremental_analysis(test_data_dir, analysis_components):
    ...
    assert len(updated_results["risk_scores"]) == len(initial_results["risk_scores"]) + 1
