...
def generate_test_message(
    text: str = None, source: str = None, metadata: Dict[str, Any] = None
) -> Dict[str, Any]:
    ...
    return {"text": text, "source": source, "metadata": metadata}


def create_test_dataset(
    size: int = 100, scam_ratio: float = 0.5, random_seed: int = 42
) -> pd.DataFrame:
    ...
    data = [generate_random_message(True) for _ in range(scam_count)] + [
...
def validate_model_prediction(
    prediction: Dict[str, Any], expected_keys: List[str] = None
) -> bool:
    """Validate model prediction format and values."""
    if expected_keys is None:
        expected_keys = ["threat_score", "confidence", "categories", "indicators"]
...
def create_test_embeddings(
    num_samples: int = 100, embedding_dim: int = 768, random_seed: int = 42
) -> np.ndarray:
...
def create_test_features(
    num_samples: int = 100, num_features: int = 10, random_seed: int = 42
) -> pd.DataFrame:
