...
def test_database_performance(large_dataset):
    ...
    results = db.query_messages(
        start_date="2025-06-01", end_date="2025-06-30", limit=1000
    )
...
def test_api_response_time(benchmark):
    ...
    def make_request():
        return api.get_analysis_results(
            start_date="2025-06-01", end_date="2025-06-30", limit=100
        )
...
def test_load_testing():
        ...
        for _ in range(100):  # Simulate 100 concurrent requests
            tasks.append(
                api.async_process_request(
                    {"text": generate_test_message(), "source": "api"}
                )
            )
    ...
    assert (
        total_time < 30.0
    )  # Should handle 100 concurrent requests in less than 30 seconds

    # Calculate response time statistics
    response_times = [r["processing_time"] for r in results]
    assert (
        np.mean(response_times) < 0.5
    )  # Average response time should be less than 500ms
    assert (
        np.percentile(response_times, 95) < 1.0
    )  # 95th percentile should be less than 1s
