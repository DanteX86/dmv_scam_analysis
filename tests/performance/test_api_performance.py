...
def test_analyze_endpoint_response_time(
    test_client, test_token, test_messages, benchmark
):
    """Test response time of analyze endpoint."""
    headers = {"Authorization": f"Bearer {test_token}"}

    def run_analyze():
        response = test_client.post("/analyze", headers=headers, json=test_messages[0])
...
def test_memory_usage(test_client, test_token, test_messages):
    ...
    for i in range(1000):
        response = test_client.post(
            "/analyze", headers=headers, json=test_messages[i % len(test_messages)]
        )
        assert response.status_code == 200
