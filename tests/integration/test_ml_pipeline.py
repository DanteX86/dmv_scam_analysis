...
def training_data():
    ...
    for _ in range(n_samples):
        ...
        is_scam = any(f in text.lower() for f in ["urgent", "click", "verify"]) and np.random.random() > 0.2
...
def test_end_to_end_training(training_data, ml_pipeline_components):
    ...
    # Split data
    train_idx = np.random.choice(len(training_data), int(0.8 * len(training_data)), replace=False)
...
def test_feature_extraction(training_data, ml_pipeline_components):
    ...
    importance = nlp_analyzer.get_feature_importance(features, training_data["label"].values[:10])
...
def test_behavioral_pattern_detection(training_data, ml_pipeline_components):
    ...
    assert all(isinstance(cluster, dict) for cluster in patterns["threat_clusters"].values())
...
def test_incremental_learning(training_data, ml_pipeline_components):
    ...
    classifier.train(
        texts=initial_data["text"].values,
        labels=initial_data["label"].values
    )
    ...
    initial_accuracy = np.mean(np.round(initial_predictions) == new_data["label"].values)
    updated_accuracy = np.mean(np.round(updated_predictions) == new_data["label"].values)
...
def test_model_explainability(training_data, ml_pipeline_components):
    ...
    assert all(isinstance(score, float) for score in explanation["feature_importance"].values())
