...
def test_ensemble_functionality(classifier, test_messages):
    ...
    print(f"Training ensemble with {len(texts)} messages and {len(set(labels))} unique labels")
...
def analyze_prediction_confidence_variance(predictions, test_messages):
    ...
    for i, (msg, mean_pred, std_pred) in enumerate(zip(test_messages, mean_preds, std_preds)):
        confidence_level = "HIGH" if std_pred < 0.1 else "MEDIUM" if std_pred < 0.2 else "LOW"
        threat_level = "HIGH" if mean_pred > 0.6 else "MEDIUM" if mean_pred > 0.3 else "LOW"
        ...
        if i < len(individual_preds[0]):
            ...
            print(f"   Individual scores: {[f'{score:.3f}' for score in individual_scores]}")
...
def compare_ensemble_vs_individual(classifier, test_messages):
    ...
    for i, (msg, ensemble_score, individual_score) in enumerate(zip(test_messages, ensemble_means, individual_scores)):
        ...
        print(f"Message {i+1}: Ensemble={ensemble_score:.3f}, Individual={individual_score:.3f}, Diff={diff:.3f} ({agreement})")

def generate_performance_report(model_performances, ensemble_results, confidence_analysis):
    ...
    if model_performances:
        ...
        if confidence_analysis:
            ...
            print(f"   Low Confidence Predictions: {stats.get('low_confidence_count', 0)}")
...
def main():
    ...
    print("- Train multiple model types (Random Forest, Gradient Boosting, SVM, Logistic Regression, Naive Bayes)")
    ...
    print(f"✅ Extracted {len(feature_data.get('feature_names', []))} features from {len(test_messages)} messages")
    
    # Test individual models
    model_performances, training_results = evaluate_individual_models(classifier, feature_data)
    
    # Test ensemble functionality
    ensemble_predictions, ensemble_results = test_ensemble_functionality(classifier, test_messages)
    ...
    confidence_analysis = analyze_prediction_confidence_variance(ensemble_predictions, test_messages)
    
    # Compare ensemble vs individual models
    compare_ensemble_vs_individual(classifier, test_messages)
    
    # Generate comprehensive report
    final_report = generate_performance_report(model_performances, ensemble_results, confidence_analysis)
    
    print("\n" + "="*60)
    print("🎉 ENSEMBLE MODEL PERFORMANCE TEST COMPLETED SUCCESSFULLY!")
    print("="*60)
    print("\nTask Requirements Fulfilled:")
    print("✅ Trained multiple model types (Random Forest, Gradient Boosting, SVM, Logistic Regression, Naive Bayes)")
