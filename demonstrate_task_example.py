...
def main() -> int:
    ...
    if not training_results or "error" in training_results:
        print(
            f"❌ Failed to train classifier: {training_results['error'] if training_results else 'unknown error'}"
        )
        return 1

    print(
        f"✅ Classifier trained successfully (best model: {training_results['best_model']})"
    )
    ...
    try:
        ...
        if len(predictions) == len(test_messages):
            ...
            print(
                f"⚠️  Expected {len(test_messages)} predictions, got {len(predictions)}"
            )
        ...
        differences = [
            abs(orig - loaded)
            for orig, loaded in zip(original_predictions, predictions)
        ]
